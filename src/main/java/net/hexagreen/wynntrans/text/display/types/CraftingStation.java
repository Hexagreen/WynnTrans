...
public class CraftingStation extends WynnDisplayText {
    ...
    public CraftingStation(Text text) {
        super(text);
        this.profession = Profession.getProfession(
                text.getString().replaceFirst("§f. §6§l", "").replaceAll("§r§f(.|\\n)+", ""));
    }
    ...
    private Text getHead() {
        ...
        head.append(profession.getIcon()).append(" ")
                .append(profession.getText().setStyle(Style.EMPTY.withColor(Formatting.GOLD).withBold(true))).append(" ")
                .append(profession.getIcon());
        return head;
    }
}
