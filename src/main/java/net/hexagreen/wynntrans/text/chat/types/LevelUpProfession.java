	...
	protected void build() {
		...
		for(int i = 4; getSiblings().size() > i; i++) {
			if(getSibling(i).getString().contains("+ Faster Gathering")) {
				resultText.append(newTranslate(parentKey + ".gatheringSpeed").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(23)).setStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)).append("\n");
				continue;
			}
			if(getSibling(i).getString().contains("+ Higher Refining")) {
				resultText.append(newTranslate(parentKey + ".gatheringRate").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(30)).setStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)).append("\n");
				continue;
			}
			if(getSibling(i).getString().contains("+ New Tool")) {
				...
				resultText.append(newTranslate(parentKey + ".newTool").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(toolName).append("\n");
				continue;
			}
			if(getSibling(i).getString().contains("+ New Resource")) {
				resultText.append(newTranslate(parentKey + ".newResource").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(19)).append("\n");
				continue;
			}
			if(getSibling(i).getString().contains("+ Higher Recipe")) {
				resultText.append(newTranslate(parentKey + ".higherRecipe").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(Text.literal(getSibling(i).getString().substring(24)).setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append("\n");
				continue;
			}
			if(getSibling(i).getString().contains("+ New Recipe")) {
				resultText.append(newTranslate(parentKey + ".newRecipe").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(Text.literal(getSibling(i).getString().substring(15)).setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append("\n");
				continue;
			}
