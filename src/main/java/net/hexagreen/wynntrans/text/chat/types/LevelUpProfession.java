...
public class LevelUpProfession extends WynnChatText implements ISpaceProvider {
    ...
    protected void build() {
        ...
        if(m2.find()) {
            ...
            Text t2 = Text.translatable(parentKey + ".nowOnIn", m2.group(1), prof).setStyle(Style.EMPTY.withColor(Formatting.YELLOW));
        ...
        for(int i = 4; getSiblings().size() > i; i++) {
            if(getSibling(i).getString().contains("+ Faster Gathering")) {
                resultText.append(Text.translatable(parentKey + ".gatheringSpeed").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(23)).setStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)).append("\n");
                continue;
            }
            if(getSibling(i).getString().contains("+ Higher Refining")) {
                resultText.append(Text.translatable(parentKey + ".gatheringRate").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(30)).setStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)).append("\n");
                continue;
            }
            ...
            if(getSibling(i).getString().contains("+ New Resource")) {
                resultText.append(Text.translatable(parentKey + ".newResource").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(getSibling(i).getString().substring(19)).append("\n");
                continue;
            }
            if(getSibling(i).getString().contains("+ Higher Recipe")) {
                resultText.append(Text.translatable(parentKey + ".higherRecipe").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(Text.literal(getSibling(i).getString().substring(24)).setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append("\n");
                continue;
            }
            if(getSibling(i).getString().contains("+ New Recipe")) {
                resultText.append(Text.translatable(parentKey + ".newRecipe").setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append(Text.literal(getSibling(i).getString().substring(15)).setStyle(Style.EMPTY.withColor(Formatting.GRAY))).append("\n");
                continue;
            }
            ...
            if(m3.find()) {
                ...
                if(m3.group(3) != null) {
                    ...
                    Text nextResource = Text.translatable(parentKey + ".nextResource", moreLevel, resourceName).setStyle(Style.EMPTY.withColor(Formatting.DARK_PURPLE));
