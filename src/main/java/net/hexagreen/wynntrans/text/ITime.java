...
public interface ITime {
    Pattern timePattern = Pattern.compile("(?:(\\d+)(?:hours|hour|h))?(?:(\\d+)(?:minutes|minute|min|m))?(?:(\\d+)(?:seconds|second|sec|s))?");
    default MutableText translateTime(String string) {
        Matcher matcher = timePattern.matcher(string.replaceAll(" ", ""));
        MutableText result = Text.empty();
        if(matcher.find()) {
            String h = matcher.group(1), m = matcher.group(2), s = matcher.group(3);
            if(h != null) {
