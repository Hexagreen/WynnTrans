...
public class ObjectiveComplete extends WynnChatText implements ICenterAligned {
    ...
    protected void build() {
        resultText = Text.empty().append("\n")
                ...
                .append(newTranslate(func + "objCompleted").setStyle(titleStyle)).append("\n");
        ...
        if(WTS.checkTranslationExist(keyObjectiveName, valObjectiveName)) {
            resultText.append(getCenterIndent(keyObjectiveName, argObjectiveName))
                    .append(newTranslate(keyObjectiveName, argObjectiveName).setStyle(objectiveNameStyle));
        }
        else {
            resultText.append(getCenterIndent(Text.literal(valObjectiveName)))
                    .append(Text.literal(valObjectiveName).setStyle(objectiveNameStyle));
        }

        resultText.append("\n\n");

        if(titleStyle.equals(Style.EMPTY.withColor(Formatting.DARK_GREEN))) {
            resultText.append(newTranslate(func + "info").setStyle(Style.EMPTY.withColor(Formatting.LIGHT_PURPLE)))
                    .append("\n");
