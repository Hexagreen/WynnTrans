    ...
    protected void build() {
        ...
        if(WTS.checkTranslationExist(keyAreaName, valAreaName)) {
            t0.append(newTranslate(keyAreaName).setStyle(styleAreaName))
                    .append(experience).setStyle(Style.EMPTY.withColor(Formatting.LIGHT_PURPLE));
        }
        else {
            t0.append(valAreaName).setStyle(styleAreaName)
                    .append(experience).setStyle(Style.EMPTY.withColor(Formatting.LIGHT_PURPLE));
        }
        ...
        for(int i = 2; inputText.getSiblings().size() > i; i++) {
            ...
            String valAreaLore = getSibling(i).getString();
